...
new_faqs_obra_nueva = [
    {"q": "¿Cuánto se tarda de media en construir una casa unifamiliar en Valencia?", "a": "Desde que se obtiene la licencia de obras, el plazo de ejecución material suele oscilar entre los 10 y 14 meses, dependiendo de los metros cuadrados, la complejidad del diseño y el sistema constructivo elegido (la construcción industrializada puede reducir este plazo a 6-8 meses)."},
    {"q": "¿Quién se encarga de solicitar las acometidas de luz y agua?", "a": "Como parte de nuestro servicio integral, nuestro equipo técnico te asesora y acompaña en la solicitud de todas las acometidas provisionales de obra y las altas definitivas con las compañías suministradoras operativas en la Comunitat Valenciana."},
    {"q": "¿Es posible hacer cambios en el diseño una vez iniciada la obra?", "a": "Sí, aunque siempre recomendamos cerrar el diseño en fase de proyecto para evitar sobrecostes y retrasos. Cualquier modificación durante la obra se presupuesta previamente, se aprueba por la propiedad y se documenta para mantener el control total del presupuesto."}
]

new_faqs_reformas = [
    {"q": "¿Tengo que abandonar mi casa durante una reforma integral?", "a": "Para reformas integrales completas que implican demoliciones, cambio de suelos, fontanería e instalaciones eléctricas, es totalmente imprescindible desalojar la vivienda por motivos de seguridad, salubridad (polvo, ruido) y tiempos de ejecución. Volverás cuando esté completamente limpia y terminada."},
    {"q": "¿Gestionáis los permisos con la comunidad de vecinos?", "a": "Nos encargamos de solicitar los permisos oficiales al Ayuntamiento, pero la comunicación con el presidente de la comunidad o administrador de fincas corre a cargo del propietario. No obstante, nosotros nos coordinamos con ellos para la protección de elementos comunes (ascensor, zaguán) y el respeto de los horarios de descanso."},
    {"q": "¿Qué pasa si aparecen vicios ocultos (tuberías podridas, vigas en mal estado) al demoler?", "a": "En pisos antiguos es posible encontrar sorpresas al descubrir estructuras o instalaciones ocultas. Si esto ocurre, paralizamos esa zona, documentamos el problema fotográficamente, proponemos la mejor solución técnica y te presentamos un presupuesto de la partida extraordinaria antes de proceder."}
]

new_faqs_rehabilitacion = [
    {"q": "¿Qué garantías ofrecéis en la impermeabilización de terrazas comunitarias?", "a": "Dependiendo del sistema utilizado (poliuretano, lámina asfáltica, EPDM), ofrecemos garantías de impermeabilización certificadas por los fabricantes que van desde los 10 hasta los 25 años, asegurando la tranquilidad de los vecinos de las últimas plantas."},
    {"q": "¿Cómo minimizáis las molestias a los vecinos durante la rehabilitación de la fachada?", "a": "Instalamos andamiajes homologados con redes de protección tupidas, protegemos los aparatos de aire acondicionado y ventanas de cada vivienda, y mantenemos una limpieza diaria de las zonas de paso. Nuestro jefe de obra está siempre localizable para resolver cualquier incidencia con los propietarios."},
...
# Update the contents in constants.ts
...
# We need to inject the seoContent and extend the faqs.
# Instead of doing complex string replacement, I will generate a small python script that parses the TS ast... wait, no.
# I can do regex replacements easily if I match the slugs.

services_updates = {
    'obra-nueva-valencia': {
        'content': content_obra_nueva,
        'new_faqs': new_faqs_obra_nueva
    },
    'reformas-integrales-valencia': {
        'content': content_reformas,
        'new_faqs': new_faqs_reformas
    },
    'rehabilitacion-edificios-valencia': {
        'content': content_rehabilitacion,
        'new_faqs': new_faqs_rehabilitacion
    }
}

for slug, data in services_updates.items():
    # 1. Add seoContent
    # Find where the service object starts
...
print("Updated constants.ts with SEO content for 3 services.")
